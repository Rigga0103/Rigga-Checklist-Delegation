...
function AccountDataPage() {
  ...
  const confirmMarkDone = async () => {
    ...
    try {
      ...
      // Prepare submission data for multiple items - Column P update
      const submissionData = selectedHistoryItems.map((historyItem) => ({
        ...
        adminProcessedDate: todayFormatted, // Store in Column P (index 15)
      }))

      const formData = new FormData()
      formData.append("sheetName", CONFIG.SHEET_NAME)
      formData.append("action", "updateAdminProcessedData") // Action for Column P
      ...
      if (result.success) {
        // UPDATED: Instead of removing items, update their Column P value in the local state
        setHistoryData((prev) =>
          prev.map((item) => {
            const isProcessedItem = selectedHistoryItems.some((selectedItem) => selectedItem._id === item._id)
            if (isProcessedItem) {
              return {
                ...item,
                col15: todayFormatted, // Update Column P with admin processed date
              }
            }
            return item
          })
        ...
        setSuccessMessage(
          `Successfully marked ${selectedHistoryItems.length} items as admin processed! Date stored: ${todayFormatted}`,
        )
                        ...
                        <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-12">
                          <div className="flex flex-col items-center">
                            <input
                              ...
                              checked={
                                filteredHistoryData.filter(item => isEmpty(item["col15"])).length > 0 &&
                                selectedHistoryItems.length === filteredHistoryData.filter(item => isEmpty(item["col15"])).length
                              }
                              onChange={(e) => {
                                const unprocessedItems = filteredHistoryData.filter(item => isEmpty(item["col15"]))
                                if (e.target.checked) {
                              ...
                              {!isEmpty(history["col15"]) ? (
                                ...
                                <div className="flex flex-col items-center">
                                  <input
                                    ...
                                    checked={true}
                                    disabled={true}
                                    title={`Admin processed on: ${history["col15"]}`}
                                  />
                              ...
                              {!isEmpty(history["col15"]) ? (
                                <div className="text-sm text-gray-900 break-words">
                                  <div className="font-medium text-green-700">
                                    {history["col15"].includes(" ") ? history["col15"].split(" ")[0] : history["col15"]}
                                  </div>
                                  {history["col15"].includes(" ") && (
                                    <div className="text-xs text-green-600">
                                      {history["col15"].split(" ")[1]}
                                    </div>
                                  )}
