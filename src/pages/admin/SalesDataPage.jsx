...
import { CheckCircle2, Upload, X, Search, History, ArrowLeft, Edit, Save, XCircle } from "lucide-react"
...
function AccountDataPage() {
  ...
  const isSubmitEnabled = useMemo(() => {
    ...
    return selectedItemsArray.every(id => additionalData[id] === "Yes" || additionalData[id] === "Not Done");
  ...
  const handleSubmit = async () => {
    ...
    // const itemsWithoutYesStatus = selectedItemsArray.filter((id) => {
    //   const status = additionalData[id];
    //   return status !== "Yes";
    // });

    // if (itemsWithoutYesStatus.length > 0) {
    //   alert(`All selected items must have status "Yes" to submit. ${itemsWithoutYesStatus.length} item(s) do not have "Yes" status.`);
    //   return;
    // }
          ...
          <div className="flex space-x-4">
            ...
            {!showHistory && (
              <button
                ...
                className={`rounded-md py-2 px-4 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition-all duration-200 ${isSubmitEnabled && !isSubmitting
                        ...
                        <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-12">
                          <div className="flex flex-col items-center">
                            <input
                              ...
                              checked={
                                filteredHistoryData.filter(item => (isEmpty(item["col15"]) || (item["col15"].toString().trim() !== "Done" && item["col15"].toString().trim() !== "Not Done"))).length > 0 &&
                                selectedHistoryItems.length === filteredHistoryData.filter(item => (isEmpty(item["col15"]) || (item["col15"].toString().trim() !== "Done" && item["col15"].toString().trim() !== "Not Done"))).length
                              }
                              onChange={(e) => {
                                const unprocessedItems = filteredHistoryData.filter(item => (isEmpty(item["col15"]) || (item["col15"].toString().trim() !== "Done" && item["col15"].toString().trim() !== "Not Done")))
                                if (e.target.checked) {
                          ...
                          <tr key={history._id} className="hover:bg-gray-50">
                                ...
                                {isInEditMode ? (
                                  ...
                                  <div className="flex items-center space-x-2">
                                    <select
                                      ...
                                      onChange={(e) => setEditedAdminStatus(prev => ({
                                        ...prev,
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div>
                                      ...
                                      ) : !isEmpty(history["col15"]) && history["col15"].toString().trim() === "Not Done" ? (
                            ...
                            {userRole === "admin" && (
                              <td className="px-3 py-4 w-12">
                                {!isEmpty(history["col15"]) && (history["col15"].toString().trim() === "Done" || history["col15"].toString().trim() === "Not Done") ? (
                                  // Already processed - show status only
                                  <div className="flex flex-col items-center">
                                    <div className={`h-4 w-4 rounded border-gray-300 ${history["col15"].toString().trim() === "Done" ? "text-green-600 bg-green-100" : "text-red-600 bg-red-100"}`}>
                                      <span className={`text-xs ${history["col15"].toString().trim() === "Done" ? "text-green-600" : "text-red-600"}`}>
                                        {history["col15"].toString().trim() === "Done" ? "✓" : "✗"}
                                      </span>
                                    </div>
                                    <span className={`text-xs mt-1 text-center break-words ${history["col15"].toString().trim() === "Done" ? "text-green-600" : "text-red-600"}`}>
