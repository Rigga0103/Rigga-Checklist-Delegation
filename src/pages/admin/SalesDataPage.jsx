...
const CONFIG = {
  ...
  PAGE_CONFIG: {
    ...
    description: "Showing tasks from the last 7 days (pending tasks only)",
    historyDescription:
  ...
  const filteredAccountData = useMemo(() => {
    ...
    if (pendingTaskDateFilter) {
      filtered = filtered.filter((account) => {
        ...
        const filterDate = new Date(pendingTaskDateFilter);
    ...
    try {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        if (hasColumnG && isColumnKEmpty && isEmpty(columnPValue)) {
          ...
          // Show tasks from last 7 days up to today
          const isWithinLast7Days =
            rowDate && rowDate >= sevenDaysAgo && rowDate <= today;
              ...
              <div className="relative w-full sm:w-64">
                ...
                */}
                <input
                  ...
                  type="date"
                  value={pendingTaskDateFilter}
                  onChange={(e) => setPendingTaskDateFilter(e.target.value)}
                  className="w-full px-3 py-2 border border-purple-200 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>
            )}
