...
function DelegationDataPage() {
  ...
  const formatDateTimeForGoogleSheets = useCallback((date) => {
    ...
    return {
      ...
      dateObject: new Date(year, date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds()),
  ...
  const convertToGoogleSheetsDateTime = useCallback((dateTimeString) => {
    ...
    if (dateTimeString.includes("/") && !dateTimeString.includes(":")) {
      ...
      const date = new Date(year, month - 1, day, currentDate.getHours(), currentDate.getMinutes(), currentDate.getSeconds())
      if (!isNaN(date.getTime())) {
        return formatDateTimeForGoogleSheets(date)
      }
    }
    ...
    return { formatted: dateTimeString, dateObject: null, iso: "", googleSheetsValue: dateTimeString }
  ...
  const parseGoogleSheetsDateTime = useCallback(
    (dateTimeStr) => {
      ...
      if (typeof dateTimeStr === "string" && dateTimeStr.match(/^\d{1,2}\/\d{1,2}\/\d{4}\s+\d{1,2}:\d{1,2}:\d{1,2}$/)) {
      ...
      if (typeof dateTimeStr === "string" && dateTimeStr.match(/^\d{1,2}\/\d{1,2}\/\d{4}$/)) {
      ...
      if (typeof dateTimeStr === "string" && dateTimeStr.startsWith("Date(")) {
        ...
        if (match) {
          ...
          return `${day.toString().padStart(2, "0")}/${(month + 1).toString().padStart(2, "0")}/${year} 00:00:00`
        }
      }
  ...
  const formatDateTimeForDisplay = useCallback(
    (dateTimeStr) => {
      ...
      if (typeof dateTimeStr === "string" && dateTimeStr.match(/^\d{2}\/\d{2}\/\d{4}\s+\d{2}:\d{2}:\d{2}$/)) {
        return dateTimeStr
      }
        ...
        await Promise.all(
          batch.map(async (id) => {
            ...
            if (nextTargetDate[id]) {
              const convertedDateTime = convertToGoogleSheetsDateTime(nextTargetDate[id])
            ...
            insertFormData.append("dateTimeMetadata", JSON.stringify(dateTimeMetadata))

            // If we have a proper datetime object for next target date, send it separately
            if (nextTargetDateTimeForGoogleSheets) {
              insertFormData.append("nextTargetDateTimeObject", nextTargetDateTimeForGoogleSheets.toISOString())
            }
                      ...
                      filteredHistoryData.map((history) => (
                        <tr key={history._id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 min-w-[140px]">
                            <div className="text-sm font-medium text-gray-900 whitespace-normal break-words">{formatDateTimeForDisplay(history["col0"]) || "—"}</div>
                          </td>
                          <td className="px-6 py-4 min-w-[100px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{history["col1"] || "—"}</div>
                          </td>
                          ...
                          <td className="px-6 py-4 min-w-[140px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{formatDateTimeForDisplay(history["col3"]) || "—"}</div>
                          </td>
                          ...
                          <td className="px-6 py-4 min-w-[120px]">
                            {history["col5"] ? (
                              ...
                              >
                                ...
                                />
                                <span className="text-xs whitespace-normal break-words">View</span>
                              </a>
                            ) : (
                          ...
                          {userRole === "admin" && (
                            <td className="px-6 py-4 min-w-[100px]">
                              <div className="text-sm text-gray-900 whitespace-normal break-words">{history["col7"] || "—"}</div>
                            </td>
                          )}
                          <td className="px-6 py-4 min-w-[120px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{history["col9"] || "—"}</div>
                          </td>
                        </tr>
                      ))
                    ) : (
                    ...
                    filteredAccountData.map((account) => {
                      ...
                      return (
                        ...
                        >{(isToday(account["col6"]) || isToday(account["col10"])) && (
                          ...
                          <td className="px-6 py-4 min-w-[100px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{account["col1"] || "—"}</div>
                          </td>
                          <td className="px-6 py-4 min-w-[120px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{account["col2"] || "—"}</div>
                          </td>
                          <td className="px-6 py-4 min-w-[120px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{account["col3"] || "—"}</div>
                          </td>
                          <td className="px-6 py-4 min-w-[120px]">
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{account["col4"] || "—"}</div>
                          </td>
                          <td className="px-6 py-4 min-w-[250px] max-w-[300px]">
                            <div
                              className="text-sm text-gray-900 whitespace-normal break-words leading-relaxed"
                          ...
                          <td className={`px-6 py-4 min-w-[140px] ${!account["col17"] ? "bg-yellow-50" : ""}`}>
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{formatDateTimeForDisplay(account["col6"])}</div>
                          </td>
                          <td className={`px-6 py-4 min-w-[140px] ${!account["col17"] ? "bg-green-50" : ""}`}>
                            <div className="text-sm text-gray-900 whitespace-normal break-words">{formatDateTimeForDisplay(account["col10"])}</div>
                          </td>
                          <td className={`px-6 py-4 min-w-[120px] ${!account["col17"] ? "bg-blue-50" : ""}`}>
                            <select
                              disabled={!isSelected}
                          ...
                          <td className={`px-6 py-4 min-w-[140px] ${!account["col17"] ? "bg-indigo-50" : ""}`}>
                            <input
                              type="date"
                                ...
                                nextTargetDate[account._id]
                                  ? (() => {
                                    ...
                                    if (dateStr && dateStr.includes("/")) {
                                      const datePart = dateStr.split(" ")[0] // Get only date part if datetime
                                      const [day, month, year] = datePart.split("/")
                          ...
                          <td className={`px-6 py-4 min-w-[200px] max-w-[250px] ${!account["col17"] ? "bg-purple-50" : ""}`}>
                            <textarea
