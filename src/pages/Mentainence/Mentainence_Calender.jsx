...
const Mentainence_Calender = ({ userRole, userName, displayName }) => {
  ...
  const getNextWorkingDates = (startDate, workingDays, frequency) => {
    ...
    if (idx === -1) return [];
    if (frequency === "daily") {
      // dates = workingDays.slice(idx);
      dates = workingDays.slice(idx).filter((date) => date.getDay() !== 0);
  ...
  const transformToTasks = (rows, sheetType) => {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (!startDate || !taskId) continue;
       ...
       // Debug: Check each task's date
    const day = startDate.getDay();
    const dayName = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][day];
    console.log(`Task ${taskId}: ${startDateStr} -> ${startDate} -> ${dayName} (${day})`);
  ...
  const generateCombinedDateMap = (
    ...
    const nameFilteredDelegation = applyNameFilter(
      pendingDelegation,
      selectedNameFilter
    );
    ...
    // NEW: Apply name filter
    const nameFilteredChecklist = applyNameFilter(
      pendingChecklist,
      selectedNameFilter
    );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const checklistResponse = await axios.get(
        `${BACKEND_URL}?sheet=Checklist_mentainence&action=fetch`,
        { timeout: 30000 }
      );
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      // Wait longer to ensure calendar is fully rendered
      await new Promise((resolve) => setTimeout(resolve, 1500));

      // Get the calendar element specifically
      ...
      const canvas = await html2canvas(calendarElement, {
        ...
        onclone: (clonedDoc, clonedElement) => {
          try {
            // Convert all oklch colors
            convertOklchToRgb(clonedDoc.body);

            // Make all calendar elements visible and properly sized
            const fcElements = clonedDoc.querySelectorAll(
              ".fc, .fc-view, .fc-daygrid, .fc-scrollgrid, .fc-daygrid-body"
            );
            ...
            // Ensure scrollgrid tables are visible
  ...
  const convertOklchToRgb = (element) => {
    ...
    elements.forEach((el) => {
      ...
      if (bgColor.includes("oklch")) {
        // Convert oklch to a safe color - use fallback colors from your theme
        // You can map specific oklch colors to their hex equivalents
  ...
  // Fallback function to convert oklch to hex based on your theme colors
  const oklchToHexFallback = (oklchString) => {
    // Map common oklch colors from your theme to hex
    const colorMap = {
      // From your gradient colors
    ...
    // Try to match based on color value
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-100 py-4 sm:py-8 px-2 sm:px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl p-2 sm:p-8 border border-indigo-100">
            ...
            /* PDF Generation Styles - ADD THIS SECTION */
            .fc {
            ...
            /* Ensure all colors are in hex/rgb format for PDF */
            ...
            >
              <FullCalendar
                ...
                eventContent={(arg) => {
                  ...
                  const checklistTasks = tasks.checklist || [];

                  if (checklistTasks.length === 0) return null;
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm">
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto flex-1 bg-gray-50">
            ...
            {/* Delegation Tasks */}
            {filteredTasks.delegation &&
              filteredTasks.delegation.length > 0 && (
...
export default Mentainence_Calender;
